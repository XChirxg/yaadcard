...
/* ==========================================================================
   AI Prompt Generator (A3 version - exactly 29 study cards)
   ========================================================================== */
...
/* ==========================================================================
   A3 Print Document Generator Engine (6x3 Grid = 18 Cards per Landscape page)
   ========================================================================== */
function generatePrintSheet() {
  ...
  p1El.innerHTML = `
    ...
    <div class="print-grid">
      <!-- Row 0: 6 card fronts -->
      ${renderCardFrontOrEmpty(sheet1Cards[0], 'grid-row: 1; grid-column: 1;')}
      ${renderCardFrontOrEmpty(sheet1Cards[1], 'grid-row: 1; grid-column: 2;')}
      ${renderCardFrontOrEmpty(sheet1Cards[2], 'grid-row: 1; grid-column: 3;')}
      ${renderCardFrontOrEmpty(sheet1Cards[3], 'grid-row: 1; grid-column: 4;')}
      ${renderCardFrontOrEmpty(sheet1Cards[4], 'grid-row: 1; grid-column: 5;')}
      ${renderCardFrontOrEmpty(sheet1Cards[5], 'grid-row: 1; grid-column: 6;')}
      
      <!-- Row 1 & 2 layout with Tuck Box -->
      ${renderCardFrontOrEmpty(sheet1Cards[6], 'grid-row: 2; grid-column: 1;')}
      ${renderCardFrontOrEmpty(sheet1Cards[7], 'grid-row: 2; grid-column: 2;')}
      <!-- Tuck Box Outside Net (spans Cols 2, 3, 4 of Row 1 & 2) -->
      <div class="tuck-box-container" style="grid-column: 3 / span 3; grid-row: 2 / span 2; width: 7.5in; height: 7.0in; display: flex; justify-content: center; align-items: center; box-sizing: border-box; background: #ffffff; --theme-color: var(--${deckTheme}-primary); --theme-color-rgb: var(--${deckTheme}-rgb);">
      ...
      ${renderCardFrontOrEmpty(sheet1Cards[8], 'grid-row: 2; grid-column: 6;')}
      
      ${renderCardFrontOrEmpty(sheet1Cards[9], 'grid-row: 3; grid-column: 1;')}
      ${renderCardFrontOrEmpty(sheet1Cards[10], 'grid-row: 3; grid-column: 2;')}
      ${renderCardFrontOrEmpty(sheet1Cards[11], 'grid-row: 3; grid-column: 6;')}
    </div>
  ...
  p2El.innerHTML = `
    ...
    <div class="print-grid">
      <!-- Row 0 backs (mirrored) -->
      ${renderCardBackOrEmpty(sheet1Cards[5], 'grid-row: 1; grid-column: 1;')}
      ${renderCardBackOrEmpty(sheet1Cards[4], 'grid-row: 1; grid-column: 2;')}
      ${renderCardBackOrEmpty(sheet1Cards[3], 'grid-row: 1; grid-column: 3;')}
      ${renderCardBackOrEmpty(sheet1Cards[2], 'grid-row: 1; grid-column: 4;')}
      ${renderCardBackOrEmpty(sheet1Cards[1], 'grid-row: 1; grid-column: 5;')}
      ${renderCardBackOrEmpty(sheet1Cards[0], 'grid-row: 1; grid-column: 6;')}
      
      <!-- Row 1 backs (mirrored) -->
      ${renderCardBackOrEmpty(sheet1Cards[8], 'grid-row: 2; grid-column: 1;')}
      <!-- Empty space where tuck box back would align (leaves inside of pack blank) -->
      <div class="tuck-box-container" style="grid-column: 2 / span 3; grid-row: 2 / span 2; width: 7.5in; height: 7.0in; border: none !important; background: transparent !important;"></div>
      ${renderCardBackOrEmpty(sheet1Cards[7], 'grid-row: 2; grid-column: 5;')}
      ${renderCardBackOrEmpty(sheet1Cards[6], 'grid-row: 2; grid-column: 6;')}
      
      <!-- Row 2 backs (mirrored) -->
      ${renderCardBackOrEmpty(sheet1Cards[11], 'grid-row: 3; grid-column: 1;')}
      ${renderCardBackOrEmpty(sheet1Cards[10], 'grid-row: 3; grid-column: 5;')}
      ${renderCardBackOrEmpty(sheet1Cards[9], 'grid-row: 3; grid-column: 6;')}
    </div>
    <div class="print-page-footer">YAADCARD — print, cut, fold, and pack</div>
  `;
  ...
  if (remainingCards.length > 0) {
    ...
    for (let i = 0; i < remainingCards.length; i += cardsPerPage) {
      ...
      frontsPageEl.innerHTML = `
        ...
        <div class="print-grid text-center"></div>
...
// Utility to dynamically calculate font sizes for front card titles based on word lengths to prevent wrapping words in the middle
function getFrontFontSizeStyle(title) {
  ...
  // Only decrease size if a single word is too long and would split/overflow the card width
  if (maxWordLength > 16) {
    size = 0.85;
  ...
  return `font-size: ${size}rem !important; word-wrap: normal !important; word-break: normal !important; white-space: normal !important;`;
}
